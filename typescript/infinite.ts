...
const randomName = (random: Random): string => {
    ...
    if (random.nextBoolean()) {
        output.push(random.nextElement(pre))
    }
    output.push(capFirst(random.nextElement(adjective)))
    output.push(capFirst(random.nextElement(subject)))
    return output.join(" ")
}
...
class Stencil implements Terminable {
    ...
    private renderPreview(): HTMLCanvasElement {
        ...
        const scale: number = (this.size * 0.5 - Stencil.PADDING) / this.radius * pixelRatio
...
(async () => {
    ...
    pattern.addEventListener("click", (event: MouseEvent) => {
        ...
        const wasActive = stencil.isActive()
        pattern.querySelectorAll("[active]")
        ...
        model.deserialize(stencil.getModel().serialize())
